...
permalink: /post/699
summary: >-
  Automate deployments to GKE with Cron and Cloud Build! Learn how to grant
  Kubernetes API access and build a simple continuous delivery pipeline.
---
...
![](https://icco.imgix.net/photos/2026/0Q74BQV1RSEKA.png)
...
![](https://icco.imgix.net/photos/2026/0Q74BQYDWSD0T.png)
...
* There is a single postgres Cloud SQL, that a very small subset of services talk to.[^graphql-layer]
...
![](https://icco.imgix.net/photos/2026/0Q74BR0ARSF26.png)
You can read the Go code that does the actual update at [https://github.com/icco/cron/blob/8cba5454db6732756ba31df10ea8e284867d9252/updater/main.go#L140-L170](https://web.archive.org/web/20260109182425/https://github.com/icco/cron/blob/8cba5454db6732756ba31df10ea8e284867d9252/updater/main.go), but the interesting thing here is giving permission to the Cron deployment to talk to the k8s master API. To do this, you'll need to create a Roll \(i.e. a permission set\), a service account \(this is something you can assign to a pod or a deployment\), and a roll binding to connect them.
...
The role has a name `cron-role` and is in the namespace `default`. The important section though is the rules. To find the correct API groups, you basically need to dig through [the API documentation](https://web.archive.org/web/20190907073442/https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.12/). The verbs are the actions on the resource that you can take.
...
[^graphql-layer]: Most services talk to a GraphQL server, [github.com/icco/graphql](https://github.com/icco/graphql), which talks to the database.
